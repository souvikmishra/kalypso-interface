...
<script lang="ts">
	...
	$: handleReference = (
		(ref?: Reference<ISeriesApi<'Candlestick'>>) => (series: ISeriesApi<'Candlestick'> | null) => {
			reference = series;
			if (ref !== undefined) {
				ref(series);
			}
